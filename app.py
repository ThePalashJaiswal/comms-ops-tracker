...
def save_data(events):
    st.session_state.skip_fetch = True   # skip next auto-fetch to avoid race
    if not GITHUB_TOKEN or not GITHUB_REPO:
...
# ── Load state ────────────────────────────────────────────────────────────────
# Re-fetch from GitHub on every rerun so all sessions stay in sync.
# Skip fetch only in the same rerun that just wrote (avoid race condition).
if "save_msg" not in st.session_state:
    st.session_state.save_msg = ""
if "show_add" not in st.session_state:
    st.session_state.show_add = False
if "skip_fetch" not in st.session_state:
    st.session_state.skip_fetch = False

if st.session_state.skip_fetch:
    st.session_state.skip_fetch = False          # reset for next cycle
else:
    st.session_state.events = load_data()        # pull latest from GitHub
